...
-user is transferred back to 

*/



function loginFacebook($db, $user_id){
    ...
    if ($user) {
      ...
      $user = getUser($db, $user['id'], 'facebook_user_id', array('me'));
...
function loginTwitter($db, $user_id){
    ...
    if(!empty($_GET['oauth_verifier']) && !empty($_SESSION['oauth_token']) && !empty($_SESSION['oauth_token_secret'])){
        ...
        if(isset($user_info)){
            ...
            if($user == 0){
                ...
                $sql = "INSERT INTO user (twitter_user_id, username, time_registered, last_visit, auth) VALUES (".
